...
test('plan contains finite ordered targets and preserves mm to pulse calibration',()=>{const {b,db,c,s}=fixture(),p=B.plan(b,db,c,s);assert.deepEqual(p.map(m=>m.target),[1000,200,400,500,200]);assert.deepEqual(p.map(m=>m.gesture),['pair','pair','down','down','up']);assert.equal(B.pulses(.005),1);});
for(const [name,change,match]of [
 ...
 ['too wide',f=>f.b.width=101,/width/],
 ['unachievable angle',f=>f.b.angle=80,/angle/],
...
test('configuration rejects fractional pulses and non-boolean sensing',()=>{assert.throws(()=>B.validateSettings({...B.settings,vMax:1.5}));assert.throws(()=>B.validateSettings({...B.settings,stopDiag:'true'}));assert.throws(()=>B.validateSettings({...B.settings,toolMax:5001}));});
