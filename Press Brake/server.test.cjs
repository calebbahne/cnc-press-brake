...
test('serves UI, proxies settings/results, rejects foreign origin and handles offline ESP', async () => {
  ...
  try {
    ...
    assert.match(html, /PRESS BRAKE/);
    const script = await (await fetch(url+'/app.js')).text();
    assert.match(script, /location.host\+'\/ws'/);
    for (const file of ['/model.js','/style.css']) assert.equal((await fetch(url+file)).status,200);
    assert.equal((await fetch(url+'/firmware/press_brake/wifi_secrets.h')).status,404);
