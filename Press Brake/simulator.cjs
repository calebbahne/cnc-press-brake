...
function createSimulator(){
  ...
  function command(socket,id,message){let token=0,op=message;const bar=message.indexOf('|');if(bar>=0){token=Number(message.slice(0,bar));op=message.slice(bar+1);}const reply=(ok,message)=>write(socket,token?{type:'result',id:token,ok,message}:{notice:message});
    ...
    if(!['goto','step','manual','home','jog'].includes(kind)||!token){reply(false,'Identified move required.');return;}
    ...
    else if(kind==='jog'){const parts=op.split(':'),sign=parts[2],requested=Number(parts[3]),maximum=i===0?cfg.vSpeed:cfg.hSpeed;if(!['+','-'].includes(sign)||!Number.isSafeInteger(requested)||requested<cfg.start||requested>maximum||parts.length!==4){reply(false,'Jog requires a direction and allowed speed.');return;}target=position[i]+(sign==='+'?1:-1)*cfg.maxTravel;speed=requested;}
    else {target=kind==='goto'?Number(value):position[i]+Number(value);if(!Number.isSafeInteger(target)||Math.abs(target)>1000000000||(kind==='goto'&&!zeroed[i])||(kind!=='manual'&&!zeroed[i]&&Math.abs(target-position[i])>200)||Math.abs(target-position[i])>cfg.maxTravel||(kind!=='manual'&&zeroed[i]&&(target<0||target>(i===0?Math.min(cfg.vMax,cfg.toolMax):cfg.hMax)))){reply(false,'Target exceeds homing / travel requirements.');return;}}
    moveId=token;holdAt=Date.now();axis=i;if(kind!=='jog')speed=i===0?cfg.vSpeed:cfg.hSpeed;reply(true,'Move accepted.');
  }
