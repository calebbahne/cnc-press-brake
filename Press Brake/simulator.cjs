...
function createSimulator(){
  ...
  function command(socket,id,message){let token=0,op=message;const bar=message.indexOf('|');if(bar>=0){token=Number(message.slice(0,bar));op=message.slice(bar+1);}const reply=(ok,message)=>write(socket,token?{type:'result',id:token,ok,message}:{notice:message});
    ...
    if(!['goto','step','home','jog'].includes(kind)||!token){reply(false,'Identified move required.');return;}
    ...
    else if(kind==='jog'){const parts=op.split(':'),sign=parts[2],requested=Number(parts[3]),maximum=i===0?cfg.vSpeed:cfg.hSpeed,limit=i===0?Math.min(cfg.vMax,cfg.toolMax):cfg.hMax;if(!zeroed[i]||!['+','-'].includes(sign)||!Number.isSafeInteger(requested)||requested<cfg.start||requested>maximum||parts.length!==4){reply(false,'Jog requires a home, direction and allowed speed.');return;}const available=sign==='+'?limit-position[i]:position[i];if(available<=0){reply(false,'Jog is at the travel boundary.');return;}target=position[i]+(sign==='+'?1:-1)*Math.min(available,cfg.maxTravel);speed=requested;}
